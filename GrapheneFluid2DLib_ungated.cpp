...
GrapheneFluid2D::GrapheneFluid2D(SetUpParameters &input_parameters) : Fluid2D(input_parameters) {
	...
	file_infix = buffer;
    /* For UNGATED-ONLY Declaration */
    // see if the core dumps cuz of this...
...
void GrapheneFluid2D::Richtmyer_ung(){
    ...
    // call these before entering -for cycle
   // cout<<"Numb: "<<Nx<<","<<Ny<<","<<dx<<","<<dy<<endl;
    GetPhi(phi_arr,Den,Nx,Ny,dx,dy);
    //cout<<"Let's make sure it doesn't get here\n "<<endl;
    //cout<<"Numb: "<<Nx<<","<<Ny<<","<<dx<<","<<dy<<endl;
    //GetPhi(phi_arr_mid,den_mid,Nx-1,Ny-1,dx,dy);  // gets updates in the same place as den_mid so it should be fine-
// BUT HOW IS THIS USED!?- not used like den_mid cuz there's no cont. enq for this- only enters in the px stuff which then influences den_mid COMPARE both cases!
...
/***************************  ************************/
			...
			flxX_mid[ks] = flx_x_avg
					-0.5f*(dt/dx)*(XMomentumFluxX_ung(point,'E') - XMomentumFluxX_ung(point,'W'))// THIS GUY....
...
/*******************************************************/
    ...
    //GetPhi(phi_arr,Den,Nx,Ny,dx,dy);// update phi_arr
    cout<<"Got_richt"<<FlxY[2]<<endl;
}


void GrapheneFluid2D::GetPhi(float phi[], float *n, int Sx,int Sy,float dx,float dy){// this *& is a very suspect thing wtf!
      
        // this top part would go into a .h file!!
        // will need to rearrange these constants sometime ofc...
        
        //float constant  =0.1f; what was before and worked!
        float constant  =0.9f; //0.6 earlier!!
        float lambda =1.0f; // for now -- put this constant as a DEFINE!
        
        float dqx = 2.*M_PI/(Sx*dx);// am I making confusion with Nx,Ny free?
        float dqy = 2.*M_PI/(Sy*dy);
        
        int n_arr = Sx * Sy;
        
        fftwf_plan_with_nthreads(2);// simulation time increases with this number??--- ya o core dump tem a ver com isto-paralelization
        
        //fftwf_complex n_in[n_arr], n_out[n_arr], V_q[n_arr], V_r[n_arr];// use malloc on these guys!-  YEA THE PROBLEM IS HERE!!--overflows stack? dynamic allocate
    	...
    	/*n_in = (fftwf_complex*) fftwf_malloc(sizeof(fftwf_complex)*n_arr + 1???);
        n_out = (fftwf_complex*) fftwf_malloc(sizeof(fftwf_complex)*n_arr);*/

        for (int i = 0; i < n_arr; i++){ // was this i++ thing it??
			n_in[i][REAL] = n[i];
			n_in[i][IMG] = 0.; 
		}	

		
		p_n = fftwf_plan_dft_2d(Sx,Sy, n_in, n_out, FFTW_FORWARD, FFTW_MEASURE); // Yea here is another problem - PC can't handle that much memory- must dynamic allocate
        //cout<<"Numb_phi44: "<<Nx<<","<<Ny<<","<<dx<<","<<dy<<","<<n_arr<<endl;
        fftwf_execute(p_n);
        fftwf_destroy_plan(p_n);

       /* V_q = (fftwf_complex*) fftwf_malloc(sizeof(fftwf_complex)*n_arr);
        V_r = (fftwf_complex*) fftwf_malloc(sizeof(fftwf_complex)*n_arr);*/

        /* Loading up of V_q */

         for (int j=0; j<Sy; j++){
            for(int i = 0; i<Sx;i++) {//the i and j on the secondary grid run differently...HOLY SHIT IT WORKED!
        ...
        fftwf_destroy_plan(p_V);
         ...
         for (int j=0; j<Sy; j++){
            for(int i = 0; i<Sx;i++) {// is this n_out attribution killing me?- shouldn't cuz it's a value...- see if this does it...-if not def j in a diff way...
		...
		/*for (int i =0; i < n_arr; i++){
			phi[i] =V_r[i][REAL]/(Nx*Ny);// full turn scaled by n
		}*/
...
float GrapheneFluid2D::XMomentumFluxX_ung(GridPoint p, char side) {//DEFINE these headers as well!
	float * phi_ptr;
	float * den_ptr;// GET_PHI has to be somewhere here!!
	float * px_ptr;
	float *dvel_ptr;
    float *oddvisco_ptr;/***/
	...
	if(p.IsMidGrid){  // se ESTÁ na grelha média tem de APONTAR pra outra grelha
		...
		phi_ptr = phi_arr; /****************************** let's try again the same guy' *******************************/
		den_ptr = den_mid;
		px_ptr = flxX_mid;
...
float GrapheneFluid2D::YMomentumFluxY_ung(GridPoint p, char side) {
	float * phi_ptr;// DEFINE this guys!
	float * den_ptr;
	float * py_ptr;
	float * dvel_ptr;
	...
	float phi =0.0f; //DEFINE this guys!
	float den =1.0f;
	float py =0.0f;
	float dvx =0.0f;
	float mass;
	if(p.IsMidGrid){
		phi_ptr = phi_arr;// Actually DEFINE THIS guys ONLY!!
		den_ptr = Den;
		py_ptr = FlxY;
		dvel_ptr = velX_dy;
        oddvisco_ptr = oddvisco_arr;
	}else{
		phi_ptr = phi_arr;// AND THIS!
		den_ptr = den_mid;
		py_ptr = flxY_mid;
    ...
    return py * py / mass + vel_fer * vel_fer * mass / 3.0f - 1.0f* phi*den - odd_visco*dvx;//check multip.const
}
